...
class SimpleStrategySettings(StrategySettings):
    ...
    def calculate_limit_memory_proposal(self, data: PodsTimeData, max_oomkill: float = 0) -> float:
    ...
    def calculate_request_memory_proposal(self, data: PodsTimeData, max_oomkill: float = 0) -> float:
        data_ = [np.max(values[:, 1]) for values in data.values()]
        if len(data_) == 0:
            return float("NaN")

        return max(
            np.max(data_) * (self.memory_request_percentage / 100),
            max_oomkill * (self.memory_request_percentage / 100),
        )
...
class CustomStrategy(BaseStrategy[SimpleStrategySettings]):
    @property
    def description(self):
        s = textwrap.dedent(f"""\
            CPU request: {self.settings.cpu_percentile}% percentile, limit: unset
            Memory request: max + {self.settings.memory_request_percentage}%, limit: max + {self.settings.memory_buffer_percentage}%

            All parameters can be customized. For example: `krr simple --rz_cpu_percentile=90 --rz_mem_req_percentile=15 --rz_mem_limit_percentile=24`
            """)

    display_name = "rz"  # The name of the strategy
